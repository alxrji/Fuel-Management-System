...
<body class="animsition">
                                    ...
                                    <table class="table table-borderless table-data3">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="?block1=<?php echo $row['user_id']; ?>" class="btn btn-outline-danger btn-sm">Block</a>
                                                    <a href="?unblock1=<?php echo $row['user_id']; ?>" class="btn btn-outline-success btn-sm">Unblock</a>
                                                </td>
                                            </tr>
                                        </tbody>
                     <?php    
  ...
  if(isset($_GET['block1'])){
    $id1 = $_GET['block1'];
    $select_user1 = "SELECT `log_id` FROM `renter` WHERE `user_id` = '$id1'";
    $select_user_result1 = mysqli_query($conn,$select_user1);
    $user1 = mysqli_fetch_array($select_user_result1);
    $logid1 = $user1['log_id'];
    $block1 = "UPDATE `login` SET `statuss`='0' WHERE `log_id` = '$logid1'";
    $block_run1 = mysqli_query($conn,$block1);
    echo '<script> alert ("Account blocked");</script>';
	  echo'<script>window.location.href="pump.php";</script>';
  }
  if(isset($_GET['unblock1'])){
    $id1 = $_GET['unblock1'];
    $select_user1 = "SELECT `log_id` FROM `renter` WHERE `user_id` = '$id1'";
    $select_user_result1 = mysqli_query($conn,$select_user1);
    $user1 = mysqli_fetch_array($select_user_result1);
    $logid1 = $user1['log_id'];
    $block1 = "UPDATE `login` SET `statuss`='1' WHERE `log_id` = '$logid1'";
    $block_run1 = mysqli_query($conn,$block1);
